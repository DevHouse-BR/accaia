...
(function ($, document, window) {
	...
	$(document).bind('cbox_complete', function(){
		...
		$('.cboxTitleDiscription').remove();
		$('#cboxTitle p').hide();

		//get title and hide the default element
		var cDescription = $('#cboxTitle p').html();
		//$('#cboxTitle').hide();
		//$('#cboxTitle').offset({top: 0, left: 0});

		if(cDescription.length > 0){
			$('<div class="cboxTitleDiscription" style="z-index:9999;">' + cDescription + '</div>').insertAfter('#colorbox');
			...
			$('.cboxTitleDiscription').offset({top: cPosition.top + cHeight, left: cPosition.left});
			$('.cboxTitleDiscription').width(cWidth);
			$('.cboxTitleDiscription').show();
		}
	});
