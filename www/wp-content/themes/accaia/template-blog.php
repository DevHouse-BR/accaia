					...
					<?php } ?>
							  ...
							  <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></h3>
					           ...
					           <div class="metadata">
								    <?php 
					                 ...
					                 <a class="post_comments" href="<?php the_permalink(); ?>#comments"><?php comments_number(__('No comments', 'kslang'), __('1 comment', 'kslang'), __('% comments', 'kslang')); ?></a>
					            </div>
									 ...
									 //Sidebar enabled	
										...
										if(has_post_thumbnail()): // POST has thumbnail
											...
											if($show_image_lightbox=='enable')
												echo '<a href="'.$org_img_url.'" class="image lightbox_blog" title="'.$post_title.'" rel="gallery"><img src="'.$url_post_img.'" alt="" class="blog_thumbnail"/></a>';
											else 
						...
						<div id="pagination-container">
							...
							<?php if (  $wp_query->max_num_pages > 1 ) : $paged = intval(get_query_var('paged')); ?>
								<?php if ( $data['wm_sidebar_enabled'] == "1" ) {?>
								<div id="pagination">										
									<div class="older"><?php next_posts_link( __('&larr; Mais Antigas', 'kslang') ); ?></div>
					
									<?php if($paged > 1) :?>
										<div class="newer"><?php previous_posts_link( __('Mais Novas &rarr;', 'kslang') ); ?></div>
									<?php endif; ?>
								</div><!-- #nav-below -->
								<?php } else { ?>
